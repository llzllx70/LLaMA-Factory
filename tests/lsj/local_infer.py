...
CLASSIFY_PROMPT_GEN = """<image>图片是电梯某个部件，已知有如下部件类别:【{type}】
...
ANSWER_PROMPT_SINGLE_GEN = """我对此图片的识别过程如下:
此图片的内容可以描述为:【{desc}】
此描述可总结为:【{conclusion}】
结合描述和总结最匹配的类别为:【{type}】
"""
        ...
        for type_, images in desc_structure.items():
            ...
            for image in images:
                ...
                dict_ = {
                    "messages": [
                        {
                            "content": f"{CLASSIFY_PROMPT_GEN.format(type=self.types)}",
                            "role": "user"
                        },
                        {
                            "content": f"{ANSWER_PROMPT_SINGLE_GEN.format(desc=image['desc'], conclusion=conslusion, type=type_)}",
                            "role": "assistant"
                        }
                    ],
